...
#####(Really) Simple Base Types
```javascript
class Model { id: string!, ts: DateTime!, name: string? }

class History<Model> extends Model { source: Model }

class Record<Model> extends Model { current: Model, history: Array<History<Model>> }

class Participant extends Model { party: Model }

class Affiliation<Model> extends Model { roster: Array<Model> } 
```
